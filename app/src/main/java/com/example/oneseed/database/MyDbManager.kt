...
class MyDbManager(context: Context) {
    ...
    fun insertToDB(name: String, coordinates: String, photo: String,
                   varieties: String, comment: String, loaded: String, result: String, datetime: String){
        val values = ContentValues().apply {
            ...
            put(MyDbClass.COLUMN_NAME_LOADED, loaded)
            put(MyDbClass.COLUMN_NAME_RESULT, result)
            put(MyDbClass.COLUMN_NAME_DATETIME, datetime)
        }
    ...
    fun readDBAllData(): ArrayList<ArrayList<String>>{
        ...
        while (cursor?.moveToNext()!!){
            ...
            dataArray.add(cursor.getString(cursor.getColumnIndex(MyDbClass.COLUMN_NAME_DATETIME)))
